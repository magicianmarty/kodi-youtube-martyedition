...
Note that the addon reads a lot of Kodi state once, at import time, into module
...
class KodiEnv(object):
    ...
    def reset(self):
        ...
        self.addon_data_dir = self._mk("profile", "addon_data", "plugin.video.youtube")
        ...
        # harness always reports the version that is actually checked out.
        self.addon_info = {
            "id": "plugin.video.youtube",
            "name": self._addon_attr("name", "YouTube"),
            "version": self._addon_attr("version", "0.0.0"),
            "path": REPO_ROOT,
            "profile": "special://profile/addon_data/plugin.video.youtube/",
