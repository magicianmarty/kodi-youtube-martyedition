...
Only the parts the addon touches outside of lib/windows are modelled with real
behaviour: window properties (the addon's cross-process signalling channel) and
...
class ListItem(object):
    ...
    # Kodi 20+. Its mere presence is a version probe: kodion.compatibility
    # picks an ISO date format when it exists and a Kodi 18 one when it does
    # not, so a stub without it silently pins the wrong behaviour.
